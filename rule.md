...
| 值类型          | 示例值         | 对应颜色            | 说明                  |
|--------------|-------------|-----------------|---------------------|
| **特殊值**      | `1`         | -               | 触发自动视图采样，系统自动检测底部颜色 |
| **ARGB整数值**  | `-16777216` | 黑色 (0xFF000000) | 直接设置导航栏为指定颜色        |
...
|-------|--------|------------------|
| **0** | 禁用模式   | 对话框窗口禁用沉浸适配      |
| **1** | 视图采样模式 | 对话框启用沉浸，使用视图采样取色 |
| **2** | SF采样模式 | 对话框启用沉浸，使用SF采样取色 |
...
```
应用级别 enable/enable31 → Activity级别 mode → 具体规则
```

### 2. 颜色来源优先级（当mode=1时）

```
color字段指定（color≠1） → SF采样（sf_sampling_mode=1） → 视图采样（color=1） → 系统默认
```
...
- 如果同时配置了 `mode=0`（禁用）和 `sf_sampling_mode=1`（强制SF采样），SF采样优先
